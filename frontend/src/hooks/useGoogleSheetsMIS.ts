...
export function useGoogleSheetsMIS() {
  ...
  const fetchAllMasterSheetData = useCallback(async () => {
    ...
    if (existingData && existingData.data.length > 0) {
      console.log(
        `📋 Using existing data for all sheets: (${existingData.data.length} records)`
      );
      ...
      setData({
        ...
        stats: null,
        fields:
          existingData.data.length > 0 ? Object.keys(existingData.data[0]) : [],
    ...
    try {
      console.log(
        "🚀 Fetching all quarterly sheet data with proper sheet dimensions..."
      );

      const allRecords = await service.fetchAllQuarterlySheetData();

      console.log(
        `✅ Successfully fetched ${allRecords.length} records for client-side filtering`
      );
  ...
  const fetchSheetData = useCallback(
    async (sheetName: string) => {
      ...
      if (existingData && existingData.data.length > 0) {
        console.log(
          `📋 Using existing data for sheet: ${sheetName} (${existingData.data.length} records)`
        );
        ...
        setData({
          ...
          fields:
            existingData.data.length > 0
              ? Object.keys(existingData.data[0])
              : [],
          lastUpdated: existingData.lastFetched,
          sheetName: sheetName,
          sheetId: null,
      ...
      try {
        ...
        console.log(
          `✅ Successfully fetched ${sheetRecords.length} records from sheet "${sheetName}"`
        );
   ...
   */
  const bulkUpdateSheetData = useCallback(
    async (sheetName: string, updates: BulkUpdateRequest) => {
      setLoading({ masterSheetData: true });
      setError({ masterSheetData: null });
      ...
      try {
        ...
        console.log(
          `✅ Bulk update completed: ${response.successful_updates} successful, ${response.failed_updates} failed`
        );
        ...
        const failureRate =
          response.total_updates > 0
            ? response.failed_updates / response.total_updates
            : 0;
        if (failureRate > 0.1) {
          console.log(
            "⚠️ High failure rate detected, refreshing data for consistency..."
          );
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        console.error(
          `❌ Error during bulk update on sheet "${sheetName}":`,
          errorMessage
        );
        setError({ masterSheetData: errorMessage });
        throw error;
  ...
  const refreshAllData = useCallback(async () => {
    try {
      await Promise.all([fetchAllMasterSheetData(), fetchMasterSheetFields()]);
    } catch (error) {
  ...
  const updateFilters = useCallback(
    (newFilters: Partial<MasterSheetListParams>) => {
...
export function useBalanceSheetStats() {
  ...
  const [loadingState, setLoadingState] = useAtom(
    googleSheetsMISLoadingStateAtom
  );
   ...
   */
  const fetchBalanceSheetStats = useCallback(
        ...
        try {
          ...
          if (cached) {
            ...
            if (cacheAge < oneHour && parsedCache.data) {
              ...
              console.log(
                "Loaded balance sheet data from cache due to network error"
              );
            }
          }
        } catch (cacheError) {
  ...
  useEffect(() => {
    // Don't fetch if there's already an error to prevent infinite loops
    if (
      !dataState.stats &&
      !loadingState.masterSheetStats &&
      !errorState.masterSheetStats
    ) {
      try {
        const cached = localStorage.getItem("balance_sheet_stats");
        if (cached) {
        ...
        console.error(
          "Error loading cached balance sheet data on mount:",
          error
        );
      }
