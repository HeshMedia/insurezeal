...
const Calculations: React.FC<CalculationProps> = ({ control, setValue }) => {
  ...
  const od_premium =
    useWatch({ control, name: "extracted_data.od_premium" }) || 0;
  const net_premium =
    useWatch({ control, name: "extracted_data.net_premium" }) || 0;
  const tp_premium =
    useWatch({ control, name: "extracted_data.tp_premium" }) || 0;
  const gross_premium =
    useWatch({ control, name: "extracted_data.gross_premium" }) || 0;
  const product_type = useWatch({
    control,
    name: "extracted_data.product_type",
  });
  const plan_type = useWatch({ control, name: "extracted_data.plan_type" });
  const commissionable_premium =
    useWatch({ control, name: "admin_input.commissionable_premium" }) || 0;
  const incoming_grid_percent =
    useWatch({ control, name: "admin_input.incoming_grid_percent" }) || 0;
  const agent_commission_given_percent =
    useWatch({ control, name: "admin_input.agent_commission_given_percent" }) ||
    0;
  const extra_grid = useWatch({ control, name: "admin_input.extra_grid" }) || 0;
  const agent_extra_percent =
    useWatch({ control, name: "admin_input.agent_extra_percent" }) || 0;
  const payment_by = useWatch({ control, name: "admin_input.payment_by" });
  const payout_on = useWatch({ control, name: "admin_input.payout_on" });
  const od_agent_payout_percent =
    useWatch({ control, name: "admin_input.od_agent_payout_percent" }) || 0;
  const tp_agent_payout_percent =
    useWatch({ control, name: "admin_input.tp_agent_payout_percent" }) || 0;
  const od_incoming_grid_percent =
    useWatch({ control, name: "admin_input.od_incoming_grid_percent" }) || 0;
  const tp_incoming_grid_percent =
    useWatch({ control, name: "admin_input.tp_incoming_grid_percent" }) || 0;
  const od_incoming_extra_grid =
    useWatch({ control, name: "admin_input.od_incoming_extra_grid" }) || 0;
  const tp_incoming_extra_grid =
    useWatch({ control, name: "admin_input.tp_incoming_extra_grid" }) || 0;
  const payment_by_office =
    useWatch({ control, name: "admin_input.payment_by_office" }) || 0;
  const cutpay_received = useWatch({ control, name: "cutpay_received" }) || 0;
  const cutpay_received_status = useWatch({
    control,
    name: "cutpay_received_status",
  });
  const agent_code = useWatch({ control, name: "admin_input.agent_code" });
  const current_cut_pay_amount =
    useWatch({ control, name: "calculations.cut_pay_amount" }) || 0;
  ...
  useEffect(() => {
    ...
    const roundedCalculatedCommissionablePremium = roundToTwo(
      calculatedCommissionablePremium
    );
    if (roundedCalculatedCommissionablePremium !== commissionable_premium) {
      setValue(
        "admin_input.commissionable_premium",
        roundedCalculatedCommissionablePremium,
        { shouldValidate: true }
      );
    }
  }, [
  ...
  useEffect(() => {
    ...
    switch (payout_on) {
      case "OD":
        receivable_from_broker = roundToTwo(
          od_premium * (incoming_grid_percent / 100)
        );
        agent_po_amt = roundToTwo(
          od_premium * (agent_commission_given_percent / 100)
        );
        break;
      case "NP":
        receivable_from_broker = roundToTwo(
          net_premium * (incoming_grid_percent / 100)
        );
        agent_po_amt = roundToTwo(
          net_premium * (agent_commission_given_percent / 100)
        );
        break;
      case "OD+TP":
        // If extra percent exists, add them; otherwise, just use the base.
        const final_od_percent =
          od_incoming_grid_percent + (od_incoming_extra_grid || 0);
        const final_tp_percent =
          tp_incoming_grid_percent + (tp_incoming_extra_grid || 0);
      ...
      default:
        ...
        receivable_from_broker = roundToTwo(
          commissionable_premium * (incoming_grid_percent / 100)
        );
        agent_po_amt = roundToTwo(
          commissionable_premium * (agent_commission_given_percent / 100)
        );
        break;
    }
    ...
    const total_receivable_from_broker = roundToTwo(
      receivable_from_broker + extra_amount_receivable_from_broker
    );
    const total_receivable_from_broker_with_gst = roundToTwo(
      total_receivable_from_broker * 1.18
    );

    const agent_extra_amount = roundToTwo(
    ...
    if (payment_by === "Agent") {
      ...
      switch (payout_on) {
        ...
        case "NP":
          ...
          break;
        case "OD+TP":
          cut_pay_amount = roundToTwo(
            od_premium + tp_premium - total_agent_po_amt
          );
          break;
        default:
          // Fallback to using commissionable premium
          cut_pay_amount = roundToTwo(
            commissionable_premium - total_agent_po_amt
          );
          break;
      }
    }
    ...
    const isCloseToCalculated =
      Math.abs(cut_pay_amount - existingCutPayAmount) < 0.01;
    ...
    if (isCloseToCalculated || existingCutPayAmount === 0) {
      setValue("calculations.cut_pay_amount", cut_pay_amount, {
        shouldValidate: true,
      });
    }
    ...
    const running_balance = roundToTwo(
      payment_by_office - total_agent_po_amt - cutpay_received + poPaidAmount
    );

    setValue("calculations.receivable_from_broker", receivable_from_broker, {
      shouldValidate: true,
    });
    setValue(
      "calculations.extra_amount_receivable_from_broker",
      extra_amount_receivable_from_broker,
      { shouldValidate: true }
    );
    setValue(
      "calculations.total_receivable_from_broker",
      total_receivable_from_broker,
      { shouldValidate: true }
    );
    setValue(
      "calculations.total_receivable_from_broker_with_gst",
    ...
    setValue("calculations.agent_po_amt", agent_po_amt, {
      shouldValidate: true,
    });
    setValue("calculations.agent_extra_amount", agent_extra_amount, {
      shouldValidate: true,
    });
    setValue("calculations.total_agent_po_amt", total_agent_po_amt, {
      shouldValidate: true,
    });
    setValue("calculations.iz_total_po_percent", iz_total_po_percent, {
      shouldValidate: true,
    });
    setValue("calculations.already_given_to_agent", already_given_to_agent, {
      shouldValidate: true,
    });
    setValue("calculations.broker_payout_amount", broker_payout_amount, {
      shouldValidate: true,
    });
    ...
    od_incoming_extra_grid,
    tp_incoming_extra_grid,
    current_cut_pay_amount,
  ]);
