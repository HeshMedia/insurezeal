...
// Define form state structure that matches the API
interface FormState {
...
const AdminInputForm = ({ onNext, onPrev }: AdminInputFormProps) => {
  ...
  // Form state that matches the API structure
  ...
  const handleInputChange = (field: keyof CutPayFormData, value: string | number | null) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  const handleCreateTransaction = async () => {
    try {
      ...
      const createRequest: FlatCreateCutPayRequest = {
        ...
        admin_child_id: formData.admin_child_id ? Number(formData.admin_child_id) : null,
  ...
  const formSections = [
    ...
    },
    {
      ...
      fields: [
        ...
        { key: 'admin_child_id', label: 'Admin Child ID', type: 'number' }
      ]
    },
    {
      title: 'Calculations',
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {section.fields.map((field) => {
                      ...
                      if (field.conditional && formData[field.conditional.field as keyof CutPayFormData] !== field.conditional.value) {
                        return null
                      }
                      ...
                      if (field.conditionalOr) {
                        const shouldShow = field.conditionalOr.some(condition => 
                          formData[condition.field as keyof CutPayFormData] === condition.value
                        )
                        if (!shouldShow) {
                          return null
                        }
                      ...
                      const renderField = (field: FormField) => {
                        if (field.type === 'select') {
                          if (field.key === 'payout_on') {
                            ...
                            return (
                              <Select
                                value={formData[field.key as keyof CutPayFormData] as string || ''}
                                onValueChange={(value) => handleInputChange(field.key as keyof CutPayFormData, value)}
                              >
                                <SelectTrigger>
                          ...
                          const rawValue = formData[field.key as keyof CutPayFormData];
                          ...
                          return (
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                handleInputChange(field.key as keyof CutPayFormData, value);
                              }}
