...
import { Database, RefreshCw, TrendingUp, FileText, DollarSign } from 'lucide-react'
...
interface AgentMISTableProps {
  agentCode: string
  onStatsUpdate?: (stats: { number_of_policies: number, running_balance: number, total_net_premium: number }) => void
}
...
export function AgentMISTable({ agentCode, onStatsUpdate }: AgentMISTableProps) {
  const {
    ...
    // Accept full React Query context to fix typing error; safely cast pageParam to number
    queryFn: async (context: { pageParam?: unknown; [key: string]: unknown }) => {
    ...
    initialPageParam: 1, // Required by React Query typing
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px] bg-gradient-to-br from-red-50 to-orange-50 rounded-2xl">
        <div className="text-center p-8 bg-white rounded-xl shadow-lg border border-red-100">
          ...
          <Button onClick={handleRefresh} variant="outline" className="border-red-300 hover:bg-red-50">
  ...
  return (
    <div className="w-full h-full max-w-full flex flex-col bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 rounded-2xl shadow-2xl border border-white/20 backdrop-blur-sm overflow-hidden">
      {/* TABLE CONTAINER WITH PROPER CONTAINMENT */}
      <div className="flex-1 w-full overflow-hidden">
        <div className="w-full h-full overflow-auto scrollbar-thin scrollbar-thumb-blue-400 scrollbar-track-blue-100">
                ...
                <TableHeader className="sticky top-0 z-10">
                    {table.getHeaderGroups().map(headerGroup => (
                        <TableRow key={headerGroup.id} className="border-b-2 border-gray-300">
                            ...
                            <TableHead className="w-16 px-4 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider bg-gray-50 border-r border-gray-300">
                            S. NO
                            </TableHead>
                            {headerGroup.headers.map(header => (
                            <TableHead
                                key={header.id}
                                className="px-4 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider bg-gray-50 border-r border-gray-300 last:border-r-0 whitespace-nowrap"
                            >
                    ...
                    <TableBody className="bg-white">
                        ...
                        >
                            ...
                            {data?.pages.reduce((acc, page, pageIndex) => {
                                ...
                                return acc + (data.pages[pageIndex - 1]?.records.length || 0) + index + 1
                            }, 0) || index + 1}
                            </TableCell>
