"""
SQLAlchemy database models for Insurezeal Backend API.

This module defines all database models for the insurance management system,
including users, policies, agents, cutpay configurations, and reporting tables.
All models are designed to work with PostgreSQL and include proper relationships,
...
class Users(Base):
    """
    User authentication table synchronized with Supabase auth.users.

    This table maintains local copies of user data from Supabase authentication
    to enable proper foreign key relationships and improve query performance.
    Data is synchronized via webhooks when users are created, updated, or deleted.

    Attributes:
        id: Primary key UUID from Supabase auth
        role: User role (admin, agent, child, etc.)
