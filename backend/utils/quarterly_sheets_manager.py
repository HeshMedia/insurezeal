...
class QuarterlySheetManager:
    ...
    def _copy_formulas_only_to_row(
        ...
        try:
            ...
            try:
                template_formulas = worksheet.batch_get(
                    [template_range], value_render_option="FORMULA"
                )[0]
                if not template_formulas or not template_formulas[0]:
                    logger.warning(
                        f"⚠️ No formulas found in template row {template_row}"
                    )
                    return True  # Not an error, just no formulas to copy

                template_formula_row = template_formulas[0]
                logger.info(
                    f"📊 Found template row with {len(template_formula_row)} cells"
                )
            except Exception as e:
                logger.warning(
                    f"⚠️ Could not get formulas from template row {template_row}: {str(e)}"
                )
            ...
            try:
                ...
                if not current_values or not current_values[0]:
                    logger.info(
                        f"ℹ️ Target row {target_row} is empty, will populate with formulas where appropriate"
                    )
                    current_value_row = [""] * num_columns  # Create empty row
                else:
                    current_value_row = current_values[0]
                    logger.info(
                        f"📋 Target row {target_row} has {len(current_value_row)} existing values"
                    )
            except Exception as e:
                logger.warning(
                    f"⚠️ Could not get current values from target row {target_row}: {str(e)}"
                )
                current_value_row = [""] * num_columns  # Create empty row
            ...
            logger.info(
                f"🧮 Processing {len(template_formula_row)} columns for formula/data decision"
            )

            for i, template_cell in enumerate(template_formula_row):
                current_value = (
                    current_value_row[i] if i < len(current_value_row) else ""
                )
                header_name = (
                    headers[i].lower() if i < len(headers) else f"column_{i+1}"
                )

                # Check if this column likely contains formulas based on header name
                is_likely_formula_column = any(
                    keyword in header_name for keyword in formula_column_keywords
                )

                # Log the decision process for first few columns and formula columns
                if i < 5 or is_likely_formula_column:
                    logger.debug(
                        f"Column {i+1} '{headers[i] if i < len(headers) else 'Unknown'}': template='{str(template_cell)[:30]}', current='{str(current_value)[:30]}', formula_column={is_likely_formula_column}"
                    )
                ...
                if template_cell and str(template_cell).startswith("="):
                    ...
                    if is_likely_formula_column:
                        ...
                        updated_formula = self._update_formula_references(
                            template_cell, template_row, target_row
                        )
                        update_row.append(updated_formula)
                        formulas_copied += 1
                        if i < 10:  # Log first 10 for debugging
                            logger.info(
                                f"✅ Column {i+1} ({header_name}): Applied original template formula"
                            )
                        ...
                        if i < 10:
                            logger.info(
                                f"🔄 Column {i+1} ({header_name}): Applied original template formula (empty data)"
                            )
                        ...
                        if i < 10:
                            logger.info(
                                f"🔄 Column {i+1} ({header_name}): Updated existing formula with original template"
                            )
                    else:
                        ...
                        if i < 10:
                            logger.info(
                                f"📝 Column {i+1} ({header_name}): Preserved data: '{str(current_value)[:20]}'"
                            )
                else:
            ...
            if update_row:
                logger.info(
                    f"🚀 Updating row {target_row} with {len(update_row)} values"
                )
                worksheet.update(
                    target_range, [update_row], value_input_option="USER_ENTERED"
                )

                logger.info(f"✅ Successfully updated row {target_row}:")
                logger.info(
                    f"   📊 Original template formulas copied: {formulas_copied}"
                )
                ...
                if formula_examples:
                    logger.info(
                        f"   🧮 Original formula examples: {'; '.join(formula_examples[:3])}"
                    )
    ...
    def _update_formula_references(
        ...
        try:
            ...
            logger.warning(
                f"Could not update formula references for '{formula}': {str(e)}"
            )
    ...
    def _improve_formula_for_text_handling(self, formula: str) -> str:
        ...
        try:
            ...
            if "=" in formula and any(op in formula for op in ["+", "-", "*", "/"]):
                ...
                if not any(
                    func in formula.upper()
                    for func in ["IFERROR", "IFNA", "ISERROR", "ISBLANK"]
                ):
                    ...
                    if (
                        len(cells) >= 2 and len(formula) < 100
                    ):  # Only process simple formulas
                        ...
                        for cell in set(cells):  # Use set to avoid duplicates
                            inner_formula = inner_formula.replace(
                                cell, f"IFNA({cell},0)"
                            )

                        improved_formula = f"=IFERROR({inner_formula},0)"
                        logger.info(
                            f"Improved formula: {formula} -> {improved_formula}"
                        )
            ...
            if "/" in formula and "IFERROR" not in formula.upper():
                ...
                logger.info(
                    f"Added division error handling: {formula} -> {improved_formula}"
                )
                return improved_formula
    ...
    def create_quarter_sheet_with_template(
        ...
        try:
            ...
            try:
                ...
                for sheet in all_sheets:
                    if (
                        "template" in sheet.title.lower()
                        or "master" in sheet.title.lower()
                    ):
                        master_sheet = sheet
                        break
                ...
                if not master_sheet:
                    logger.warning(
                        "No master template sheet found, creating sheet with headers only"
                    )
                    # Just add headers if no template is found
                    headers = self.create_quarterly_sheet_headers()
                    new_sheet.update(
                        "A1:1", [headers], value_input_option="USER_ENTERED"
                    )
                    return {
                        "success": True,
            ...
            try:
                ...
                if headers_data:
                    new_sheet.update(
                        headers_range, headers_data, value_input_option="USER_ENTERED"
                    )
            ...
            try:
                ...
                sample_formulas = master_sheet.batch_get(
                    [sample_range], value_render_option="FORMULA"
                )[0]

                if sample_formulas and sample_formulas[0]:
                    # Use formulas if available, otherwise use values
                    sample_data = []
                    for i, (value, formula) in enumerate(
                        zip(
                            (
                                sample_values[0]
                                if sample_values and sample_values[0]
                                else []
                            ),
                            (
                                sample_formulas[0]
                        ...
                        if (
                            ...
                            sample_data.append(
                                formula
                            )  # Use original formula unchanged
                        else:
                            sample_data.append(
                    ...
                    new_sheet.update(
                        sample_range, [sample_data], value_input_option="USER_ENTERED"
                    )
                    logger.info(
                        f"Copied sample data and original template formulas to {quarter_name}"
                    )
                elif sample_values and sample_values[0]:
                    # Just copy values if no formulas
                    new_sheet.update(
                        sample_range, sample_values, value_input_option="USER_ENTERED"
                    )
                    logger.info(f"Copied sample data to {quarter_name}")

            except Exception as e:
                logger.warning(
                    f"Could not copy sample data from master template: {str(e)}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Could not apply formatting to {quarter_name}: {str(e)}"
                )
    ...
    def test_formula_copying(
        self, sheet_name: Optional[str] = None, target_row: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"🧪 Testing formula copying on sheet '{sheet_name}', row {target_row}"
            )
            ...
            template_formula_count = sum(
                1 for cell in template_formula_row if str(cell).startswith("=")
            )

            # Add some test data to the target row first
            test_data = ["Test Policy"] + [""] * (
                num_columns - 1
            )  # Just put something in first column
            target_range = f"A{target_row}:{last_col}{target_row}"
            worksheet.update(
                target_range, [test_data], value_input_option="USER_ENTERED"
            )
            ...
            # Check the results
            try:
                result_formulas = worksheet.batch_get(
                    [target_range], value_render_option="FORMULA"
                )[0]
            ...
            result_formula_count = sum(
                1 for cell in result_formula_row if str(cell).startswith("=")
            )

            # Create detailed report
            formula_examples = []
            for i, (template_cell, result_cell) in enumerate(
                zip(template_formula_row, result_formula_row)
            ):
