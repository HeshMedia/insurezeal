    ...
    def get_current_quarter_sheet(self) -> Optional[gspread.Worksheet]:
        ...
        try:
            ...
            if self.sheet_exists(quarter_name):
                ...
                logger.warning(f"Current quarter sheet {quarter_name} does not exist. Sheet creation is handled by Google Apps Script.")
                return None
    ...
    def route_new_record_to_specific_quarter(self, record_data: Dict[str, Any], quarter: int, year: int, operation_type: str = "CREATE") -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Quarter sheet {quarter_name} not found. Sheet creation is handled by Google Apps Script.")
                return {"success": False, "error": f"Quarter sheet {quarter_name} does not exist. Sheets must be created via Google Apps Script."}
    ...
    def check_quarter_transition(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.sheet_exists(quarter_name):
                logger.warning(f"Current quarter sheet {quarter_name} doesn't exist - sheet creation is handled by Google Apps Script")
                return {
