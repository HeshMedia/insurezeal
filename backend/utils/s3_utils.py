...
def build_cloudfront_url(key: str) -> str:
    domain = (CLOUDFRONT_DOMAIN or "").strip().lstrip("https://").lstrip("http://").rstrip("/")
    if not domain:
...
def generate_presigned_put_url(key: str, content_type: Optional[str] = None, expires_in: Optional[int] = None) -> str:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to generate S3 URL: {str(e)}")


def put_object(key: str, body: bytes, content_type: str):
    try:
        get_s3_client().put_object(Bucket=AWS_S3_BUCKET, Key=key, Body=body, ContentType=content_type)
    except ClientError as e:
