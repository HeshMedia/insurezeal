"""
Google Sheets integration utilities for Insurezeal Backend API.

This module provides comprehensive Google Sheets integration for the insurance platform,
enabling synchronized data management between the application database and Google Sheets.
It handles policy data synchronization, quarterly report generation, and real-time
updates between the platform and external spreadsheets.

Key Features:
- Policy data synchronization with Google Sheets
- Quarterly report automation
- Policy number normalization for consistent matching
- Batch operations for efficient data handling
- Error handling and retry mechanisms for reliable sync
"""
...
def normalize_policy_number_for_sheets(policy_number: Any) -> str:
    """
    Normalize policy number for consistent comparison across different data sources.

    This function standardizes policy numbers from various insurance companies
    that may have different formats, ensuring consistent matching between
    the application database and Google Sheets.

    Supported Formats:
    - D195264389 (Go Digit standard format)
    - 12-1806-0006746459-00 (hyphen-separated format)
    - 3004/372635895/00/000 (slash-separated format)
    - 'P300655564669 (quoted format from CSV imports)
    - VVT0186550000100 (alphanumeric format)
    - 201520010424700079900000 (pure numeric format)

    Args:
        policy_number: Raw policy number from any source (str, int, float, or None)

    Returns:
        str: Normalized policy number (uppercase, whitespace removed, quotes stripped)

    Note:
        This normalization must match the function used in universal_records/helpers.py
        to ensure consistent policy matching across the entire system.
    """
    ...
    try:
        ...
        # Remove leading/trailing quotes and apostrophes (common in CSV imports)
        normalized = normalized.strip("'\"'")

        # Remove internal whitespace while preserving legitimate separators
        # Policy numbers may legitimately contain hyphens, slashes, or backslashes
        normalized = (
            normalized.replace(" ", "")
            .replace("\t", "")
            .replace("\n", "")
...
class GoogleSheetsSync:
    ...
    def _find_next_empty_row(self, worksheet) -> int:
        ...
        try:
            ...
            logger.info(
                f"Google Sheets utility - Found next empty row: {next_row} (last data row was: {last_row_with_data})"
            )
            return next_row
    ...
    def sync_child_id_request(
        self, child_id_data: Dict[str, Any], action: str = "CREATE"
    ):
        ...
        def _sync():
            ...
            logger.info(
                f"Synced child ID request {child_id_data.get('id')} to Google Sheets at row {next_row}"
            )

        self._safe_sync(_sync)

    def sync_cutpay_transaction(
        self, cutpay_data: Dict[str, Any], action: str = "CREATE"
    ):
        ...
        def _sync():
            ...
            logger.info(
                f"Synced cut pay transaction {cutpay_data.get('id')} to CutPay sheet at row {next_row}"
            )
    ...
    def sync_to_master_sheet(self, cutpay_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not master_sheet:
                return {
                    "success": False,
                    "error": "Failed to get or create Master worksheet",
                }
            ...
            if row_id:
                ...
                master_sheet.update(
                    update_range, [row_data], value_input_option="USER_ENTERED"
                )
                logger.info(
                    f"Updated Master sheet row {row_number} for transaction {cutpay_id}"
                )
            else:
                ...
                logger.info(
                    f"Added new Master sheet row {row_number} for transaction {cutpay_id}"
                )
            ...
            logger.error(
                f"Failed to sync CutPay transaction {cutpay_data.get('id')} to Master sheet: {str(e)}"
            )
    ...
    def _sync_to_quarterly_sheet(self, cutpay_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            quarterly_result = quarterly_manager.route_new_record_to_current_quarter(
                cutpay_data
            )
    ...
    def sync_policy(self, policy_data: Dict[str, Any], action: str = "CREATE"):
        ...
        def _sync():
            ...
            logger.info(
                f"Synced policy {policy_data.get('policy_number')} to Policies sheet at row {next_row}"
            )
    ...
    def _sync_policy_to_master_sheet(self, policy_data: Dict[str, Any]):
        ...
        field_mappings = {
            ...
            "Child ID/ User ID [Provided by Insure Zeal]": policy_data.get(
                "child_id", ""
            ),
            ...
            "Major Categorisation( Motor/Life/ Health)": policy_data.get(
        ...
        master_sheet.update(
            cell_range, [master_row_data], value_input_option="USER_ENTERED"
        )

        logger.info(
            f"Synced policy {policy_data.get('policy_number')} to Master sheet at row {next_row}"
        )
    ...
    def sync_cutpay_to_sheets(self, cutpay_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not cutpay_sheet:
                return {
                    "success": False,
                    "error": "Failed to get or create CutPay worksheet",
                }
            ...
            if row_id:
                ...
                logger.info(
                    f"Updated CutPay sheet row {row_number} for transaction {cutpay_id}"
                )
            else:
                ...
                row_number = next_row_number
                logger.info(
                    f"Added new CutPay sheet row {row_number} for transaction {cutpay_id}"
                )

            return {"success": True, "row_id": str(row_number), "sheet_name": "CutPay"}

        except Exception as e:
            logger.error(
                f"Failed to sync CutPay transaction {cutpay_data.get('id')} to sheets: {str(e)}"
            )
            return {"success": False, "error": str(e)}
    ...
    def _prepare_cutpay_row_data(self, cutpay_data: Dict[str, Any]) -> List[Any]:
        ...
        keys_in_order = [
            ...
            "remarks",
            "company",
            "synced_to_cutpay_sheet",
            "synced_to_master_sheet",
            "notes",
            "created_at",
            "updated_at",
        ]
    ...
    def _prepare_master_sheet_row_data(self, cutpay_data: Dict[str, Any]) -> List[Any]:
        ...
        field_mappings = {
            ...
            "Child ID/ User ID [Provided by Insure Zeal]": cutpay_data.get(
                "admin_child_id", ""
            ),
            ...
            "Major Categorisation( Motor/Life/ Health)": cutpay_data.get(
                "major_categorisation", ""
            ),
            ...
            "Extra Amount Receivable from Broker": cutpay_data.get(
                "extra_amount_receivable_from_broker", ""
            ),
            " Total Receivable from Broker ": cutpay_data.get(
                "total_receivable_from_broker", ""
            ),
            ...
            " Total Receivable from Broker Include 18% GST ": cutpay_data.get(
                "total_receivable_from_broker_with_gst", ""
            ),
    ...
    def _get_cutpay_sheet_headers(self) -> List[str]:
        ...
        return [
            ...
            "Extra Amount Receivable",
            "Total Receivable",
            "Total Receivable with GST",
            "CutPay Amount",
            "Agent PO Amount",
            "Agent Extra Amount",
            "Total Agent PO",
            ...
            "IZ Total PO %",
            "Broker PO %",
            "Broker Payout Amount",
            "Invoice Status",
            "Remarks",
            "Company",
            # System Fields
            "Synced to CutPay Sheet",
            "Synced to Master Sheet",
            "Notes",
            "Created At",
            "Updated At",
        ]
        ...
        async with AsyncSessionLocal() as db:
            ...
            if cutpay_data.get("insurer_id"):
                ...
                if insurer_name:
                    ...
                    logger.info(
                        f"Resolved insurer_id {cutpay_data['insurer_id']} to name: {insurer_name}"
                    )
            ...
            if cutpay_data.get("broker_id"):
                ...
                if broker_name:
                    ...
                    logger.info(
                        f"Resolved broker_id {cutpay_data['broker_id']} to name: {broker_name}"
                    )
...
async def get_master_sheet_data(
    ...
    try:
        ...
        master_sheet = google_sheets_sync._get_or_create_worksheet(
            "Master", google_sheets_sync._get_master_sheet_headers()
        )
        if not master_sheet:
            return []
        ...
        if insurer_name:
            ...
            for record in records:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error comparing insurer name '{record_insurer}' with '{insurer_name}': {str(e)}"
                    )
                    continue
...
async def update_master_sheet_record(
    policy_number: str, updated_data: Dict[str, Any]
) -> bool:
    ...
    try:
        ...
        for i, record in enumerate(all_records):
            existing_policy = normalize_policy_number_for_sheets(
                record.get("Policy number", "")
            )
        ...
        # Update the row
        range_to_update = (
            f"A{row_index}:{google_sheets_sync._col_to_a1(len(headers))}{row_index}"
        )
        master_sheet.update(range_to_update, [row_data])
        logger.info(
            f"Updated master sheet record for policy '{policy_number}' at row {row_index}"
        )
        return True

    except Exception as e:
        logger.error(
            f"Error updating master sheet record for policy '{policy_number}': {str(e)}"
        )
        return False
...
async def add_master_sheet_record(record_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        logger.info(
            f"Added new master sheet record for policy {record_data.get('Policy number', 'unknown')}"
        )
        return True
...
def sync_policy_to_master_sheet(policy_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return google_sheets_sync._safe_sync(
        google_sheets_sync._sync_policy_to_master_sheet, policy_data
    )
...
def sync_cutpay_to_master_sheet(cutpay_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return google_sheets_sync._safe_sync(
        google_sheets_sync.sync_to_master_sheet, cutpay_data
    )
