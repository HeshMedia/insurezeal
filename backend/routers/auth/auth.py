...
async def register(
    ...
    try:
        ...
        logger.info(f"User created in Supabase: {supabase_user_id}. Webhook will handle local database creation.")
...
async def supabase_webhook(
    ...
    try:
        ...
        logger.info(f"Webhook received - Type: {webhook_data.type}, Table: {webhook_data.table}")
        logger.info(f"User ID: {webhook_data.record.id}, Email: {webhook_data.record.email}")
        ...
        # Success response
        response_data = {"status": "success", "message": f"Processed {event_type} event for user {user_data.id}"}
        ...
        logger.error(f"HTTP Exception in webhook: {http_exc.status_code} - {http_exc.detail}")
...
async def upsert_user_from_supabase(db: AsyncSession, user_data):
    ...
    try:
        ...
        if existing_user:
            ...
            logger.info(f"Successfully updated existing user {user_data.id} in local database")
        else:
            # Create new user
            ...
            logger.info(f"Successfully created new user {user_data.id} in local database")
        ...
        await db.commit()
        logger.info(f"Database transaction committed successfully for user {user_data.id}")
        
    except Exception as e:
        logger.error(f"Error in upsert_user_from_supabase for user {user_data.id}: {str(e)}", exc_info=True)
        logger.error(f"Error type: {type(e).__name__}")
        try:
            await db.rollback()
...
async def create_user_profile_from_webhook(db: AsyncSession, user_data):
    ...
    try:
        ...
        logger.info(f"Extracted metadata: {list(metadata.keys()) if metadata else 'None'}")
        ...
        logger.info(f"Profile data - username: {username}, first_name: {first_name}, last_name: {last_name}, role: {role}")
        
        if not username:
            logger.warning(f"No username found in metadata for user {user_data.id}")
            logger.info("Available metadata keys:", list(metadata.keys()) if metadata else "No metadata")
            return
        ...
        db.add(new_profile)
        logger.info(f"Successfully created user profile for webhook user {user_data.id} with username {username}")
        
    except Exception as e:
        logger.error(f"Error in create_user_profile_from_webhook for user {user_data.id}: {str(e)}", exc_info=True)
        logger.error(f"Error type: {type(e).__name__}")
