"""
Universal Records router for Insurezeal Backend API.

This module handles universal record uploads and processing with insurer-specific
mapping capabilities for data reconciliation and system synchronization. It provides
functionality to upload CSV files from various insurance companies and map them
to a standardized format for processing.

Key Features:
- Universal CSV record upload with insurer-specific mapping
- Data reconciliation and validation
- CSV preview and validation before processing
- Insurer mapping management and configuration
- Reconciliation report generation and tracking
- Integration with Google Sheets for data synchronization
- Support for multiple insurance company data formats

The system handles the complexity of different insurance company data formats
by using configurable mapping rules to transform incoming data into a
...
async def get_available_insurers(
    ...
    """
    Get list of available insurer mappings for universal record processing.

    This endpoint returns all insurance companies that have configured
    header mappings for universal record processing. These mappings allow
    the system to transform insurer-specific CSV formats into standardized
    internal records.

    Returns:
        AvailableInsurersResponse: List of available insurers with mapping configurations

    Note:
        Requires admin-level access as this information is used for system
        configuration and data processing workflows.
    """
