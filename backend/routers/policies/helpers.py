...
async def resolve_broker_code_to_details(db: AsyncSession, broker_code: str) -> tuple[Optional[int], Optional[str]]:
...
async def resolve_insurer_code_to_details(db: AsyncSession, insurer_code: str) -> tuple[Optional[int], Optional[str]]:
...
def prepare_complete_policy_sheets_data(policy_data: Any, policy_db_record: Any = None, broker_name: str = "", insurer_name: str = "") -> Dict[str, Any]:
    """
    ...
    sheets_data.update({
        ...
        "Child ID/ User ID [Provided by Insure Zeal]": getattr(policy_data, 'child_id', "") or "",
        "Insurer /broker code": getattr(policy_data, 'broker_code', "") or getattr(policy_data, 'insurer_code', "") or "",
        "Policy Start Date": getattr(policy_data, 'start_date', "").isoformat() if getattr(policy_data, 'start_date', None) else "",
        "Policy End Date": getattr(policy_data, 'end_date', "").isoformat() if getattr(policy_data, 'end_date', None) else "",
        "Booking Date(Click to select Date)": getattr(policy_db_record, 'booking_date', "").isoformat() if policy_db_record and getattr(policy_db_record, 'booking_date', None) else "",
        "Broker Name": broker_name or getattr(policy_data, 'broker_name', "") or "",
        "Insurer name": insurer_name or getattr(policy_data, 'insurance_company', "") or "",
    })
    
    # Policy and Product Information
    sheets_data.update({
        "Major Categorisation( Motor/Life/ Health)": getattr(policy_data, 'major_categorisation', "") or "",
        "Product (Insurer Report)": getattr(policy_data, 'product_insurer_report', "") or "",
    ...
    sheets_data.update({
        ...
        "GST Amount": getattr(policy_data, 'gst_amount', 0) or getattr(policy_data, 'gst', 0) or 0,
    ...
    sheets_data.update({
        ...
        "Formatted Policy number": getattr(policy_data, 'formatted_policy_number', "") or "",
    ...
    sheets_data.update({
        ...
        "Actual Agent_PO%": getattr(policy_data, 'agent_commission_given_percent', 0) or 0,
        "Agent_PO_AMT": 0,
        "Agent_Extra%": 0,
        "Agent_Extr_Amount": 0,
        "Payment By Office": getattr(policy_data, 'payment_by_office', 0) or 0,
        "PO Paid To Agent": getattr(policy_data, 'total_agent_payout_amount', 0) or 0,
...
def prepare_complete_policy_sheets_data_for_update(policy_data: Any, policy_db_record: Any, broker_name: str = "", insurer_name: str = "") -> Dict[str, Any]:
    """
    ...
    sheets_data["Child ID/ User ID [Provided by Insure Zeal]"] = policy_db_record.child_id or ""
    sheets_data["Agent Code"] = policy_db_record.agent_code or ""
    
    # Always include policy number from database record first
    logger.info(f"🔍 POLICY DEBUG Helper: Database record policy_number = {repr(policy_db_record.policy_number)}")
    ...
    if hasattr(policy_data, 'policy_number') and policy_data.policy_number:
        if policy_data.policy_number.strip():
            logger.info(f"🔍 POLICY DEBUG Helper: Overriding with update policy_number = {repr(policy_data.policy_number)}")
            sheets_data["Policy number"] = policy_data.policy_number
        else:
            logger.info(f"🔍 POLICY DEBUG Helper: Keeping database policy_number = {repr(sheets_data['Policy number'])}")
    ...
    if hasattr(policy_data, 'start_date') and policy_data.start_date:
        sheets_data["Policy Start Date"] = policy_data.start_date.isoformat() if hasattr(policy_data.start_date, 'isoformat') else str(policy_data.start_date)
    
    if hasattr(policy_data, 'end_date') and policy_data.end_date:
        sheets_data["Policy End Date"] = policy_data.end_date.isoformat() if hasattr(policy_data.end_date, 'isoformat') else str(policy_data.end_date)
    ...
    try:
        ...
        for db_field, response_field in db_field_mapping.items():
            if hasattr(policy_obj, db_field):
                ...
                elif response_field in ['booking_date', 'policy_start_date', 'policy_end_date'] and value:
