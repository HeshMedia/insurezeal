...
This module handles all policy-related operations including policy creation,
updates, AI-powered document extraction, file uploads, and policy management.
It integrates with AI services for document processing and provides comprehensive
CRUD operations for insurance policies.

Key Features:
- AI-powered PDF policy extraction using Gemini
- Policy CRUD operations with role-based access control
- File upload and management with S3 integration
- Policy number validation and duplicate checking
- Batch policy operations and filtering
- Integration with external insurance company systems
"""
...
# Role-based access control decorators for policy operations
...
async def extract_pdf_data_endpoint(
    ...
    """
    Extract policy data from uploaded PDF using AI-powered document processing.

    This endpoint uses Google Gemini AI to analyze insurance policy PDF documents
    and extract structured data including policy numbers, dates, coverage amounts,
    insurer information, and other key policy details.

    Process:
    1. Validates uploaded file is a PDF
    2. Processes PDF content using AI extraction
    3. Returns structured policy data for review and manual verification
    4. Provides confidence score for extraction accuracy

    Args:
        file: PDF file upload containing insurance policy document
        current_user: Authenticated user (requires policy write permission)
        db: Database session
        _rbac_check: Role-based access control verification

    Returns:
        AIExtractionResponse: Extracted policy data with confidence score

    Raises:
        HTTPException: If file is invalid, empty, or extraction fails

    Note:
        Extracted data should be reviewed before creating actual policy records.
        AI extraction provides a starting point but may require human verification
        for complex or non-standard policy documents.
    """
    try:
    ...
    **Requires policy write permission**
    ...
    try:
        ...
        if document_type == "policy_pdf":
            ...
            existing_urls = [
                doc.get("url") if isinstance(doc, dict) else doc
