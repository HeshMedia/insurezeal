"""
Management Information System (MIS) Helper Functions for Insurezeal Backend API.

This module provides comprehensive helper functions for MIS operations including
data retrieval, bulk updates, reporting, and integration with Google Sheets.
It handles complex data processing operations for management reporting and
business intelligence across the insurance platform.

Key Features:
- Master sheet data management and synchronization
- Bulk update operations for efficient data processing
...
Business Logic:
- Policy data aggregation and reporting
...
class MISHelpers:
    """
    Comprehensive helper class for Management Information System operations.

    This class provides methods for data processing, reporting, and integration
    with external systems for business intelligence and management reporting.
    It handles complex data operations with performance optimization and
    error handling for reliable MIS operations.

    Key Responsibilities:
        - Master sheet data management and processing
    ...
    async def get_master_sheet_data(
        ...
        try:
            ...
            master_sheet = self.sheets_client._get_or_create_worksheet(
                "Master", self.sheets_client._get_master_sheet_headers()
            )
            ...
            for idx, row_data in enumerate(all_data):
                record = self._convert_row_to_record(
                    row_data, idx + 2
                )  # +2 because row 1 is headers, and we're 0-indexed
            ...
            if filter_by:
                for field_name, filter_values in filter_by.items():
                    if filter_values:  # filter_values is now a list
                        ...
                        clean_values = [
                            v.strip().lower() for v in filter_values if v and v.strip()
                        ]
                        if clean_values:
                            all_records = [
                                record
                                ...
                                and any(
                                    filter_val
                                    in str(getattr(record, field_name)).lower()
                                    for filter_val in clean_values
                                )
                            ]
                logger.info(
                    f"Field filters applied, {len(all_records)} records remaining"
                )
            ...
            if sort_by and hasattr(MasterSheetRecord, sort_by):
                ...
                try:
                    all_records.sort(
                        key=lambda x: (
                            str(getattr(x, sort_by) or "").lower()
                            if getattr(x, sort_by)
                            else ""
                        ),
    ...
    async def get_quarterly_sheet_data(
        ...
        try:
            ...
            quarterly_records = quarterly_manager.get_all_records_from_quarter_sheet(
                quarter, year
            )
            ...
            for record_dict in quarterly_records:
                try:
                    ...
                    if len(all_records) == 0:  # Log only for first record to avoid spam
                        logger.info(
                            f"Debug: Available fields in {sheet_name}: {list(converted_record.keys())}"
                        )
                    ...
                    if len(all_records) < 3:  # Only log first few failed records
                        sample_fields = {
                            k: v
                            for i, (k, v) in enumerate(record_dict.items())
                            if i < 5
                        }
            ...
            if search and search.strip():
                ...
                all_records = filtered_records
                logger.info(
                    f"Search '{search}' filtered to {len(all_records)} records in {sheet_name}"
                )
            ...
            if filter_by:
                ...
                logger.info(
                    f"Field filters applied to {sheet_name}, {len(all_records)} records remaining"
                )
            ...
            if sort_by and hasattr(MasterSheetRecord, sort_by):
                ...
                try:
                    ...
                    logger.info(
                        f"Records in {sheet_name} sorted by {sort_by} ({sort_order})"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to sort {sheet_name} by {sort_by}: {str(e)}"
                    )
            ...
            logger.info(
                f"Returning page {page} with {len(paginated_records)} records from {sheet_name}"
            )
            ...
            logger.error(
                f"Error getting quarterly sheet data for Q{quarter}-{year}: {str(e)}"
            )
            return MasterSheetResponse(
                records=[], total_count=0, page=page, page_size=page_size, total_pages=0
            )

    async def bulk_update_master_sheet(
        self, updates: List[BulkUpdateField], admin_user_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Processing bulk update for {len(updates_by_record)} records with {len(updates)} total field updates"
            )
            ...
            for record_id, record_updates in updates_by_record.items():
                try:
                    ...
                    logger.error(
                        f"Error updating record {record_id}: {str(record_error)}"
                    )
                    for update in record_updates:
                        results.append(
                            BulkUpdateResult(
            ...
            logger.info(
                f"Bulk update completed: {successful_updates} successful, {failed_updates} failed in {processing_time:.2f}s"
            )
    ...
    async def bulk_update_quarterly_sheet(
        ...
        try:
            ...
            if policy_number_col_index is None:
                error_msg = (
                    f"Policy Number column not found in quarterly sheet {sheet_name}"
                )
                logger.error(error_msg)
                return {
                    "message": error_msg,
                ...
                try:
                    ...
                    for idx, row in enumerate(data_rows):
                        if idx < len(data_rows) and policy_number_col_index < len(row):
                            if (
                                str(row[policy_number_col_index]).strip()
                                == str(policy_number).strip()
                            ):
                                row_index = idx
                                row_number = (
                                    idx + 3
                                )  # Actual row number in sheet (1-based, +3 because we skip header and dummy row)
                                row_found = True
                                break

                    if not row_found:
                        logger.error(
                            f"Policy number {policy_number} not found in quarterly sheet {sheet_name}"
                        )
                        for update in record_updates:
                            results.append(
                                BulkUpdateResult(
                    ...
                    if updated_row != current_row:
                        ...
                        logger.info(
                            f"Updated row {row_number} for policy {policy_number} in quarterly sheet {sheet_name}"
                        )

                except Exception as record_error:
                    logger.error(
                        f"Error updating policy {policy_number} in quarterly sheet {sheet_name}: {str(record_error)}"
                    )
                    for update in record_updates:
                        results.append(
                            BulkUpdateResult(
            ...
            logger.info(
                f"Quarterly sheet {sheet_name} bulk update completed: {successful_updates} successful, {failed_updates} failed in {processing_time:.2f}s"
            )
            ...
            logger.error(
                f"Error in quarterly sheet {quarter}-{year} bulk update: {str(e)}"
            )
            processing_time = time.time() - start_time
    ...
    async def get_master_sheet_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully retrieved {len(summary_data)} records from Summary sheet"
            )
            return response
    ...
    async def get_broker_sheet_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Broker sheet has {len(headers)} columns and {len(data_rows)} data rows"
            )
            ...
            logger.info(
                f"Successfully retrieved {len(broker_data)} records from Broker sheet"
            )
            return response
    ...
    async def _find_record_row(self, worksheet, record_id: str) -> Optional[int]:
        ...
        try:
            ...
            for idx, cell_value in enumerate(
                id_column[1:], start=2
            ):  # Start from row 2
    ...
    async def get_agent_mis_data(
        self, agent_code: str, page: int = 1, page_size: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for record in all_data:
                ...
                record_agent_code = str(
                    record.get("Agent Code", record.get("agent_code", ""))
                ).strip()
                ...
                match_status = (
                    str(
                        record.get(
                            "MATCH",
                            record.get(
                                "Match Status", record.get("match_status", "TRUE")
                            ),
                        )
                    )
                    .upper()
                ...
                logger.info(
                    f"Record agent code: '{record_agent_code}', Match status: '{match_status}', Target: '{agent_code}'"
                )
            ...
            logger.info(
                f"Filtered to {len(filtered_records)} records for agent: {agent_code}"
            )
            ...
            for record in filtered_records:
                try:
                    ...
                    balance_val = record.get(
                        "Running Balance", record.get("running_balance", "0")
                    )
                    if balance_val and str(balance_val).strip():
                        balance_str = (
                            str(balance_val).replace(",", "").replace("₹", "").strip()
                        )
                        if balance_str and balance_str != "":
                            running_balance += float(balance_str)
                            logger.info(
                                f"Added balance: {balance_str} -> running total: {running_balance}"
                            )
                except (ValueError, TypeError) as e:
                    logger.warning(
                        f"Could not parse running balance '{balance_val}': {e}"
                    )
                ...
                try:
                    ...
                    if net_val and str(net_val).strip():
                        ...
                        if net_str and net_str != "":
                            ...
                            logger.info(
                                f"Added net premium: {net_str} -> running total: {total_net_premium}"
                            )
                except (ValueError, TypeError) as e:
                    logger.warning(f"Could not parse net premium '{net_val}': {e}")

            logger.info(
                f"Final statistics - Running Balance: {running_balance}, Net Premium: {total_net_premium}"
            )
    ...
    def _convert_record_to_agent_mis_record(
        ...
        try:
            ...
            agent_mis_record = {
                ...
                "reporting_month": record.get(
                    "Reporting Month", record.get("reporting_month")
                ),
                ...
                "formatted_policy_number": record.get(
                    "Formatted Policy Number", record.get("formatted_policy_number")
                ),
                "customer_name": record.get(
                    "Customer Name", record.get("customer_name")
                ),
                "customer_phone_number": record.get(
                    "Customer Phone Number", record.get("customer_phone_number")
                ),
                "major_categorisation": record.get(
                    "Major Categorisation", record.get("major_categorisation")
                ),
                "product_insurer_report": record.get(
                    "Product Insurer Report", record.get("product_insurer_report")
                ),
                "product_type": record.get("Product Type", record.get("product_type")),
                "plan_type": record.get("Plan Type", record.get("plan_type")),
                "gross_premium": record.get(
                    "Gross Premium", record.get("gross_premium")
                ),
                "net_premium": record.get("Net Premium", record.get("net_premium")),
                "registration_number": record.get(
                    "Registration No", record.get("registration_number")
                ),
                "make_model": record.get("Make Model", record.get("make_model")),
                "model": record.get("Model", record.get("model")),
                "agent_commission_perc": record.get(
                    "Agent Commission %", record.get("agent_commission_perc")
                ),
                "agent_po_amount": record.get(
                    "Agent PO Amount", record.get("agent_po_amount")
                ),
                "total_agent_po": record.get(
                    "Total Agent PO", record.get("total_agent_po")
                ),
                "running_balance": record.get(
                    "Running Balance", record.get("running_balance")
                ),
                "already_given_to_agent": record.get(
                    "Already Given to Agent", record.get("already_given_to_agent")
                ),
    ...
    async def get_quarterly_sheet_agent_data(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(all_data)} total records from Q{quarter}-{year} sheet"
            )
            ...
            logger.info(
                f"Found {len(filtered_records)} records for agent {agent_code} in Q{quarter}-{year}"
            )
        ...
        try:
            ...
            for record in all_data:
                ...
                record_agent_code = str(
                    record.get(
                        "Agent Code",
                        record.get("agent_code", record.get("Agent_Code", "")),
                    )
                ).strip()
    ...
    async def get_quarterly_sheet_agent_filtered_data(
        ...
        try:
            ...
            try:
                ...
                if not all_values or len(all_values) < 2:
                    logger.warning(
                        f"No data found in quarterly sheet Q{quarter}-{year}"
                    )
                    return {
                        "records": [],
                        "total_count": 0,
                ...
                logger.info(
                    f"Retrieved {len(all_data)} total records from Q{quarter}-{year} using manual parsing"
                )
                ...
                try:
                    ...
                    if duplicates:
                        ...
                        logger.error(
                            "No duplicate headers found, but get_all_records() still failing"
                        )
            ...
            for record in all_data:
                ...
                match_status = (
                    str(record.get("Match", record.get("MATCH", "TRUE")))
                    .upper()
                    .strip()
                )
                    ...
                    if not match_only or match_status == "TRUE":
                        ...
                        try:
                            running_bal = float(
                                str(
                                    record.get(
                                        "Running Bal", record.get("Running Balance", 0)
                                    )
                                ).replace(",", "")
                                or 0
                            )
                        ...
                        try:
                            net_premium = float(
                                str(
                                    record.get(
                                        "Net premium", record.get("Net Premium", 0)
                                    )
                                ).replace(",", "")
                                or 0
                            )
                            ...
                            commissionable_premium = float(
                                str(
                                    record.get(
                                        ...
                                        record.get("commissionable_premium", 0),
                                    )
                                ).replace(",", "")
                                or 0
                            )
            ...
            logger.info(
                f"Found {len(filtered_records)} filtered records for agent {agent_code} in Q{quarter}-{year}"
            )
            ...
            logger.error(
                f"Error fetching quarterly sheet agent filtered data: {str(e)}"
            )
            return {
                "records": [],
                "total_count": 0,
