...
async def get_my_mis_data(
    ...
    **New Fields Support:**
    ...
    try:
        ...
        logger.info(
            f"Fetching quarterly MIS data for agent: {agent_code} (user: {user_id}), Q{quarter}-{year}"
        )
        ...
        try:
            ...
            if summary_sheet:
                ...
                logger.info(
                    f"Retrieved {len(summary_records)} records from Summary sheet for stats"
                )

                # Find agent's data in Summary sheet
                for record in summary_records:
                    if (
                        str(record.get("Agent Code", "")).strip()
                ...
                if summary_stats:
                    ...
                    logger.warning(
                        f"No Summary sheet data found for agent: {agent_code}"
                    )
            else:
        ...
        if summary_stats:
            ...
            stats = AgentMISStats(
                number_of_policies=int(
                    safe_float(summary_stats.get("Policy Count (True)", 0))
                ),
                running_balance=safe_float(
                    summary_stats.get("Running Balance (True)", 0.0)
                ),
                total_net_premium=safe_float(
                    summary_stats.get("Net Premium (True)", 0.0)
                ),
                commissionable_premium=safe_float(
                    summary_stats.get("Commissionable Premium (True)", 0.0)
                ),
            )
