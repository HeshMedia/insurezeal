"""
MIS (Management Information System) router for Insurezeal Backend API.

This module provides comprehensive reporting and data analysis endpoints for
the insurance platform. It handles quarterly reports, agent performance data,
master sheet operations, and various export functionalities for management
and compliance purposes.

Key Features:
- Quarterly sheet data management and retrieval
- Agent MIS reports with performance metrics
- Master sheet statistics and analytics
- CSV and ZIP export functionality for compliance
- Policy document management and tracking
- Integration with Google Sheets for quarterly reporting
...
async def get_quarter_sheet_data(
    ...
    quarter: Optional[int] = Query(
        ...
        ge=1,
        le=4,
        description="Quarter number (1-4) to fetch data from specific quarterly sheet",
    ),
    year: Optional[int] = Query(
        None,
        ge=2020,
        le=2030,
        description="Year to fetch data from specific quarterly sheet",
    ),
    search: Optional[str] = Query(None, description="Search across key fields"),
    # Multiple filtering options - support arrays
    agent_code: Optional[List[str]] = Query(
        None, description="Filter by agent codes (can specify multiple)"
    ),
    insurer_name: Optional[List[str]] = Query(
        None, description="Filter by insurer names (can specify multiple)"
    ),
    policy_number: Optional[List[str]] = Query(
        None, description="Filter by policy numbers (can specify multiple)"
    ),
    reporting_month: Optional[List[str]] = Query(
        None, description="Filter by reporting months (can specify multiple)"
    ),
    child_id: Optional[List[str]] = Query(
        None, description="Filter by child IDs (can specify multiple)"
    ),
    broker_name: Optional[List[str]] = Query(
        None, description="Filter by broker names (can specify multiple)"
    ),
    product_type: Optional[List[str]] = Query(
        None, description="Filter by product types (can specify multiple)"
    ),
    plan_type: Optional[List[str]] = Query(
        None, description="Filter by plan types (can specify multiple)"
    ),
    make_model: Optional[List[str]] = Query(
        None, description="Filter by make/models (can specify multiple)"
    ),
    model: Optional[List[str]] = Query(
        None, description="Filter by models (can specify multiple)"
    ),
    gvw: Optional[List[str]] = Query(
        None, description="Filter by GVW values (can specify multiple)"
    ),
    rto: Optional[List[str]] = Query(
        None, description="Filter by RTO codes (can specify multiple)"
    ),
    state: Optional[List[str]] = Query(
        None, description="Filter by states (can specify multiple)"
    ),
    fuel_type: Optional[List[str]] = Query(
        None, description="Filter by fuel types (can specify multiple)"
    ),
    cc: Optional[List[str]] = Query(
        None, description="Filter by CC values (can specify multiple)"
    ),
    age_year: Optional[List[str]] = Query(
        None, description="Filter by age in years (can specify multiple)"
    ),
    # Sorting options
    sort_by: Optional[str] = Query(
        None,
        description="Field to sort by (e.g., reporting_month, agent_code, insurer_name, etc.)",
    ),
    ...
    **Returns:**
    ...
    try:
        ...
        if (quarter is not None and year is None) or (
            quarter is None and year is not None
        ):
            raise HTTPException(
        ...
        logger.info(
            f"Fetching {data_source} data - Page: {page}, Size: {page_size}, Search: '{search}', Filters: {filters}, Sort: {sort_params}"
        )
        ...
        if quarter is not None and year is not None:
            ...
            logger.info(
                f"Successfully retrieved {len(result.records)} records from {data_source}"
            )
        ...
        data_source_text = (
            f"quarterly sheet Q{quarter}-{year}" if quarter and year else "master sheet"
        )
...
async def update_quarterly_sheet_records(
    ...
    **Returns:**
    ...
    try:
        ...
        logger.info(
            f"Starting quarterly sheet update for {len(update_request.records)} records by admin {admin_user_id} on {sheet_name}"
        )
        ...
        logger.info(
            f"Converted {len(update_request.records)} records to {len(bulk_updates)} field updates"
        )
        ...
        logger.info(
            f"Quarterly sheet update completed on {sheet_name}: {result['successful_updates']} successful, {result['failed_updates']} failed"
        )
        ...
        logger.error(
            f"Error in update_quarterly_sheet_records for Q{quarter}-{year}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    **Use Cases:**
    ...
    try:
        ...
        if "error" not in broker_stats:
            ...
            logger.info(
                f"Successfully retrieved broker sheet data with {broker_stats.get('total_rows', 0)} rows"
            )
        else:
            logger.warning(
                f"Could not retrieve broker sheet data: {broker_stats.get('error', 'Unknown error')}"
            )
...
async def get_master_sheet_fields(
    quarter: Optional[int] = Query(
        ...
        ge=1,
        le=4,
        description="Quarter number (1-4) to get quarterly sheet fields",
    ),
    year: Optional[int] = Query(
        None, ge=2020, le=2030, description="Year to get quarterly sheet fields"
    ),
    current_user=Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _rbac_check=Depends(require_admin_read),
    ...
    **Note:** This route now returns quarterly sheet headers by default instead of master sheet headers,
    ...
    try:
        ...
        if quarter is not None and year is not None:
            ...
            logger.info(
                f"Retrieved {len(headers)} headers from quarterly sheet {sheet_name}"
            )
...
async def export_quarterly_sheet_data(
    quarters: str = Query(
        ...,
        description="Comma-separated quarters (e.g., '1,2' or '1'). Each quarter requires corresponding year.",
    ),
    years: str = Query(
        ...,
        description="Comma-separated years (e.g., '2025,2025' or '2025'). Must match the number of quarters.",
    ),
    ...
    **Note:** Large datasets are automatically handled. XLSX format recommended for multiple quarters.
    ...
    try:
        ...
        logger.info(
            f"Exporting quarterly data for quarters: {quarter_list}, years: {year_list}, format: {format}"
        )
        ...
        for quarter, year in zip(quarter_list, year_list):
            ...
            try:
                ...
                elif isinstance(quarterly_result, dict) and quarterly_result.get(
                    "records"
                ):
                ...
                try:
                    summary_result = await mis_helpers.get_quarterly_summary_data(
                        quarter=quarter, year=year, agent_code=agent_code
                    )
                    ...
                    if quarterly_records:
                        ...
                        for record in quarterly_records:
                            ...
                            try:
                                gross_premium = (
                                    float(
                                        str(record.get("Gross premium", "0"))
                                        .replace(",", "")
                                        .replace("₹", "")
                                    )
                                ...
                                running_balance = (
                                    float(
                                        str(record.get("Running Bal", "0"))
                                        .replace(",", "")
                                        .replace("₹", "")
                                    )
                                    if record.get("Running Bal")
                                    else 0.0
                                )
                                commissionable_premium = (
                                    float(
                                        str(record.get("Commissionable Premium", "0"))
                                        .replace(",", "")
                                        .replace("₹", "")
                                    )
                                    if record.get("Commissionable Premium")
                                    else 0.0
                                )
                                ...
                                logger.debug(
                                    f"Error converting financial values for record: {e}"
                                )
                                continue
                        ...
                        logger.info(
                            f"Created basic summary for {len(summary_result)} agents in {sheet_name}"
                        )
                    else:
                        logger.info(
                            f"No quarterly records found to create summary for {sheet_name}"
                        )
                        summary_result = []

                all_quarterly_data[sheet_name] = quarterly_records
                all_summary_data[f"{sheet_name}_Summary"] = (
                    summary_result if summary_result else []
                )

                logger.info(
                    f"Retrieved {len(quarterly_records)} records from {sheet_name}"
                )
        ...
        if format == "json":
            return JSONResponse(
                content={
                    ...
                    "export_info": {
                        ...
                        "total_sheets": len(quarter_list)
                        * 2,  # quarterly + summary for each
            ...
            with zipfile.ZipFile(zip_path, "w") as zip_file:
                ...
                for sheet_name, records in all_quarterly_data.items():
                    if records:
                        ...
                        headers = (
                            ...
                            else [f"col_{i}" for i in range(len(records[0]))]
                        )
                        ...
                        for record in records:
                            writer.writerow(
                                ...
                                else dict(zip(headers, record))
                            )

                        zip_file.writestr(
                            f"{sheet_name}_quarterly_data.csv", csv_content.getvalue()
                        )
                ...
                for summary_name, summary_records in all_summary_data.items():
                    if summary_records:
                        csv_content = io.StringIO()
                        headers = (
                            list(summary_records[0].keys())
                            if isinstance(summary_records[0], dict)
                            else [f"col_{i}" for i in range(len(summary_records[0]))]
                        )
                        writer = csv.DictWriter(csv_content, fieldnames=headers)
                        writer.writeheader()
            ...
            try:
                ...
                logger.warning(
                    f"Pandas/openpyxl not available for XLSX export: {str(e)}"
                )
...
async def get_agent_mis_data(
    ...
    **Returns:**
    ...
    try:
        logger.info(
            f"Fetching quarterly MIS data for agent: {agent_code}, Q{quarter}-{year}"
        )
        ...
        if not quarterly_result:
            logger.warning(
                f"No quarterly data found for agent: {agent_code} in Q{quarter}-{year}"
            )
            quarterly_result = {
                "records": [],
                "total_count": 0,
    ...
    try:
        ...
        if not agent_code:
            ...
            try:
                ...
                logger.warning(
                    f"Could not retrieve oldest quarter sheet name: {str(e)}"
                )

            return AgentMISResponse(
                records=[],
                stats=AgentMISStats(
                    number_of_policies=0, running_balance=0.0, total_net_premium=0.0
                ),
                total_count=0,
                page=page,
                page_size=page_size,
        ...
        logger.info(
            f"Fetching quarterly MIS data for agent: {agent_code} (user: {user_id}) from Q{quarter}-{year}"
        )
        ...
        if not quarterly_result:
            logger.warning(
                f"No quarterly MIS data found for agent: {agent_code} in Q{quarter}-{year}"
            )
            # Get the oldest quarter sheet name even for empty response
            oldest_quarter_sheet = None
            try:
        ...
        for record_dict in quarterly_result["records"]:
            ...
            mapped_record = {
                "booking_date": safe_str(
                    record_dict.get("Booking Date(Click to select Date)")
                    or record_dict.get("booking_date")
                ),
                "policy_start_date": safe_str(
                    record_dict.get("Policy Start Date")
                    or record_dict.get("policy_start_date")
                ),
                "policy_end_date": safe_str(
                    record_dict.get("Policy End Date")
                    or record_dict.get("policy_end_date")
                ),
                "policy_number": safe_str(
                    record_dict.get("Policy number") or record_dict.get("policy_number")
                ),
                "insurer_name": safe_str(
                    record_dict.get("Insurer name") or record_dict.get("insurer_name")
                ),
                "broker_name": safe_str(
                    record_dict.get("Broker Name") or record_dict.get("broker_name")
                ),
                "gross_premium": safe_str(
                    record_dict.get("Gross premium") or record_dict.get("gross_premium")
                ),
                "net_premium": safe_str(
                    record_dict.get("Net premium") or record_dict.get("net_premium")
                ),
                "commissionable_premium": safe_str(
                    record_dict.get("Commissionable Premium")
                    or record_dict.get("commissionable_premium")
                ),
                "agent_total_po_amount": safe_str(
                    record_dict.get("Agent Total PO Amount")
                    or record_dict.get("agent_total_po_amount")
                ),
                "actual_agent_po_percent": safe_str(
                    record_dict.get("Actual Agent_PO%")
                    or record_dict.get("actual_agent_po_percent")
                ),
            }
        ...
        stats = AgentMISStats(
            ...
            running_balance=quarterly_result.get("stats", {}).get(
                "running_balance", 0.0
            ),
            total_net_premium=quarterly_result.get("stats", {}).get(
                "total_net_premium", 0.0
            ),
            commissionable_premium=quarterly_result.get("stats", {}).get(
        ...
        logger.error(
            f"Error in get_my_mis_data for user {user_id}, Q{quarter}-{year}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_policy_documents(
    policy_number: str = Query(
        ...,
        description="Policy number to search for (supports special characters like HDFC/TW/2025/CUT128)",
    ),
    db: AsyncSession = Depends(get_db),
    ...
    **Access:** Requires admin read permissions
    ...
    try:
        ...
        logger.error(
            f"Error fetching policy documents for policy {policy_number}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
