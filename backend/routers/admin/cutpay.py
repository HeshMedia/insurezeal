"""
CutPay Administration Router for Insurezeal Backend API.

This module implements comprehensive API endpoints for the CutPay administrative
workflow, including policy management, commission calculations, payment processing,
and reconciliation operations. It provides admin-level access to financial
operations and policy lifecycle management.

Key Features:
- Policy search, filtering, and bulk operations
- Commission calculation and payment processing
- Excel import/export functionality for policy data
- Payment reconciliation and status management
- Cut payment workflows with approval mechanisms
- Integration with Google Sheets for reporting
- Bulk policy updates and status changes

Business Logic:
The CutPay system handles critical financial operations including:
- Commission calculations based on policy parameters
- Payment approval and processing workflows
- Policy status transitions and lifecycle management
...
- Financial data handling with proper validation
- Audit logging for all financial operations
- Secure file upload and processing
- Data integrity checks for financial calculations

Dependencies:
- SQLAlchemy: Database operations for policy and payment data
- FastAPI: API framework with file upload support
- Excel processing: Policy data import/export
- Google Sheets: External reporting integration
...
from models import CutPay, Insurer, Broker, CutPayAgentConfig
...
async def upload_policy_document(
    ...
    try:
        ...
        logger.info(
            f"🔵 PRESIGNED: Processing document_type='{document_type}' for policy '{policy_number}' (CutPay ID {cutpay_id})"
        )
        ...
        if document_type_clean == "policy_pdf":
            ...
            logger.info(
                f"❌ PRESIGNED: ENTERING additional_documents branch for type: '{document_type_clean}'"
            )
            if not cutpay.additional_documents:
                cutpay.additional_documents = {}
                logger.info(
                    f"❌ PRESIGNED: Initialized empty additional_documents dict"
                )
            ...
            updated_additional_docs = (
                dict(cutpay.additional_documents) if cutpay.additional_documents else {}
            )
            updated_additional_docs[document_type_clean] = document_url
            cutpay.additional_documents = updated_additional_docs

            logger.info(
                f"❌ PRESIGNED: additional_documents['{document_type_clean}'] updated from '{old_value}' to '{document_url}'"
            )
            logger.info(
                f"❌ PRESIGNED: New additional_documents dict: {cutpay.additional_documents}"
            )
        ...
        logger.info(
            f"Generated presigned URL for policy '{policy_number}' (CutPay ID {cutpay_id}), key: {key}"
        )
        ...
        logger.error(
            f"Failed to upload document for policy '{policy_number}': {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
