"""
Quarterly Sheets Management Router

Provides API endpoints for managing quarterly sheets, balance carryover,
and quarterly transitions.
"""
...
from dependencies.rbac import (
    ...
    require_admin_quarterly_sheets_delete
)
...
async def auto_refresh_carryover(
    source_quarter: int = Query(..., ge=1, le=4, description="Source quarter that was updated (1-4)"),
    ...
    try:
        ...
        success = quarterly_manager.auto_refresh_dependent_quarters(source_quarter, source_year)
...
async def route_record_to_current_quarter(
    ...
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=500,
                detail=result.get("error", "Failed to route record")
            )
...
async def preview_balance_carryover(
    quarter: int,
    year: int
):
    ...
    try:
        ...
        prev_quarter_name, prev_quarter, prev_year = quarterly_manager.get_previous_quarter_info(quarter, year)
        ...
        headers = quarterly_manager.create_quarterly_sheet_headers()
        carryover_preview = quarterly_manager._process_balance_carryover(prev_data, headers)
        ...
        return {
            ...
            "preview": carryover_preview[:10] if carryover_preview else []  # Show first 10 for preview
        }
...
async def manual_balance_carryover(
    ...
    try:
        ...
        source_sheet_name = quarterly_manager.get_quarterly_sheet_name(source_quarter, source_year)
        target_sheet_name = quarterly_manager.get_quarterly_sheet_name(target_quarter, target_year)
        ...
        if not quarterly_manager.sheet_exists(target_sheet_name):
            target_worksheet = quarterly_manager.create_quarterly_sheet(target_quarter, target_year)
            ...
            target_worksheet = quarterly_manager.spreadsheet.worksheet(target_sheet_name)
        
        # Apply balance carryover
        headers = quarterly_manager.create_quarterly_sheet_headers()
        quarterly_manager._apply_balance_carryover(target_worksheet, target_quarter, target_year, headers)
...
async def quarterly_system_health_check():
    ...
    try:
        ...
        sheets_connected = quarterly_manager.client is not None and quarterly_manager.spreadsheet is not None
        ...
        template_sheet_name = quarterly_manager.master_template_sheet_name if template_sheet else None
        ...
        credentials_exists = os.path.exists(quarterly_manager.credentials_path) if quarterly_manager.credentials_path else False
        ...
        return {
            ...
            "system_status": "healthy" if all([
                sheets_connected,
                master_template_exists
            ]) else "degraded",
            "recommendations": [
                "Create 'Master Template' sheet if missing" if not master_template_exists else None,
                "Create 'Record Mapper' sheet for mapping functionality" if not record_mapper_exists else None,
                "Check Google Sheets credentials" if not sheets_connected else None,
                f"Create current quarter sheet: {quarter_name}" if not current_quarter_exists else None
            ]
        }
...
async def setup_initial_quarterly_sheet(
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(status_code=500, detail=result.get("error", "Failed to setup initial sheet"))
        ...
        for i, header in enumerate(default_headers):
            if "Total Receivable from Broker" in header and "Include 18% GST" in header:
                ...
                for j, h in enumerate(default_headers):
                    if h == "Total Receivable from Broker" and "Include 18% GST" not in h:
                ...
                sample_formulas[i] = "=IF(ROW()=2,0,INDIRECT(ADDRESS(ROW()-1,COLUMN()))+M2-AM2)"
        
        # Add sample formulas to row 2
        if any(formula for formula in sample_formulas):
            template_worksheet.update('A2', [sample_formulas], value_input_option='USER_ENTERED')
