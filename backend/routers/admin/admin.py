"""
Admin router for Insurezeal Backend API.

This module provides administrative functionality for managing the insurance platform,
including agent management, statistics, child ID requests, and system administration.
...
- Child ID request management and approval
...
async def list_all_agents(
    ...
    search: Optional[str] = Query(
        None, description="Search by name, email, or agent code"
    ),
...
async def get_all_child_requests(
    ...
    search: Optional[str] = Query(
        None, description="Search by location, email, or child ID"
    ),
    current_user=Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    ...
    **Admin only endpoint**
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Invalid UUID format for request_id: '{request_id}' - Error: {str(uuid_error)}"
            )
            raise HTTPException(
        ...
        user_query = select(UserProfile).where(
            UserProfile.user_id == child_request.user_id
        )
        user_result = await db.execute(user_query)
        user_profile = user_result.scalar_one_or_none()
        ...
        if user_profile:
            ...
            req_dict["agent_code"] = user_profile.agent_code
            logger.info(
                f"Agent info for child request {request_id}: name='{agent_name}', code='{user_profile.agent_code}'"
            )
        else:
            req_dict["agent_name"] = None
            req_dict["agent_code"] = None
            logger.warning(
                f"No user profile found for user_id: {child_request.user_id}"
            )
...
async def promote_agent_to_admin(
    ...
    try:
        ...
        logger.info(
            f"Updated role in both UserProfile and Users tables for user {user_id} to admin"
        )
        ...
        return schemas.UserRoleUpdateResponse(
            success=True,
            message=(
                "Successfully promoted agent to admin in both local database and Supabase auth"
                if supabase_updated
                else "Promoted to admin in local database, but failed to sync with Supabase auth. User can still access admin features."
            ),
            user_id=user_uuid,
            new_role="admin",
