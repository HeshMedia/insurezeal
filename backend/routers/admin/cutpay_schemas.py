...
class ExtractedPolicyData(BaseModel):
    ...
    registration_number: Optional[str] = Field(
        None, description="Vehicle registration number"
    )
    ...
    start_date: Optional[str] = Field(
        None, description="Policy start date (YYYY-MM-DD)"
    )
...
class AdminInputData(BaseModel):
    ...
    incoming_grid_percent: Optional[float] = Field(
        None, description="Commission rate from insurer"
    )
    agent_commission_given_percent: Optional[float] = Field(
        None, description="Rate paid to agent"
    )
    extra_grid: Optional[float] = Field(
        None, description="Additional commission percentage"
    )
    commissionable_premium: Optional[float] = Field(
        None, description="Base amount for commission calculation"
    )

    # Payment Configuration
    payment_by: Optional[str] = Field(None, description="Who handles customer payment")
    payment_method: Optional[str] = Field(
        None, description="Payment method if InsureZeal pays"
    )
    payout_on: Optional[str] = Field(None, description="Payout calculation basis")
    agent_extra_percent: Optional[float] = Field(
        None, description="Additional agent commission percentage"
    )
    payment_by_office: Optional[float] = Field(
        None, description="Who pays agent payout"
    )
...
class CalculationResult(BaseModel):
    ...
    receivable_from_broker: Optional[float] = Field(
        None, description="Commission from broker"
    )
    extra_amount_receivable_from_broker: Optional[float] = Field(
        None, description="Extra commission"
    )
    total_receivable_from_broker: Optional[float] = Field(
        None, description="Total commission"
    )
    total_receivable_from_broker_with_gst: Optional[float] = Field(
        None, description="Total with GST"
    )
    ...
    iz_total_po_percent: Optional[float] = Field(
        None, description="InsureZeal total payout percentage"
    )
    already_given_to_agent: Optional[float] = Field(
        None, ge=0, description="Amount already paid to agent"
    )
    broker_payout_amount: Optional[float] = Field(
        None, ge=0, description="Payout amount for the broker"
    )
...
class CalculationRequest(BaseModel):
    ...
    gross_premium: Optional[float] = Field(
        None, description="Gross premium for calculations"
    )
    net_premium: Optional[float] = Field(
        None, description="Net premium for calculations"
    )
    ...
    # Commission configuration
    incoming_grid_percent: Optional[float] = Field(
        None, description="Incoming grid percentage"
    )
    extra_grid: Optional[float] = Field(None, description="Extra grid percentage")
    commissionable_premium: Optional[float] = Field(
        None, description="Commissionable premium"
    )
    agent_commission_given_percent: Optional[float] = Field(
        None, description="Agent commission percentage"
    )
    agent_extra_percent: Optional[float] = Field(
        None, description="Agent extra percentage"
    )
...
class CutPayCreate(BaseModel):
    ...
    policy_pdf_url: Optional[str] = Field(
        None, max_length=500, description="Policy PDF URL"
    )
    additional_documents: Optional[Dict[str, Any]] = Field(
        None, description="Additional documents"
    )

    # Extracted data (populated by AI extraction)
    extracted_data: Optional[ExtractedPolicyData] = Field(
        None, description="PDF extracted data"
    )

    # Admin input data
    admin_input: Optional[AdminInputData] = Field(
        None, description="Admin manual input"
    )

    # Calculated data (auto-computed)
    calculations: Optional[CalculationResult] = Field(
        None, description="Calculated amounts"
    )
    ...
    cluster: Optional[str] = Field(
        None, max_length=100, description="Business cluster/region"
    )
    ...
    def validate_invoice_status_update(cls, v):
        if v is not None:
            ...
            if v not in allowed_statuses:
                raise ValueError(
                    f"Invoice status must be one of: {', '.join(allowed_statuses)}"
                )
        return v
...
class CutPayDatabaseResponse(BaseModel):
    ...
    additional_documents: Optional[Dict[str, Any]] = Field(
        None, description="Additional document URLs as JSON"
    )
    ...
    child_id_request_id: Optional[UUID] = Field(
        None, description="Child ID request foreign key"
    )
    ...
    quarter: Optional[int] = Field(
        None,
        description="Quarter number (1-4) where record is located in Google Sheets",
    )
    year: Optional[int] = Field(
        None, description="Year where record is located in Google Sheets"
    )
    quarter_sheet_name: Optional[str] = Field(
        None, description="Google Sheets quarter name (e.g., 'Q3-2025')"
    )
...
class BulkUpdateItem(BaseModel):
    ...
    cutpay_id: int = Field(
        ..., description="The ID of the CutPay transaction to update"
    )
    update_data: CutPayUpdate = Field(
        ..., description="The data to update for this transaction"
    )
...
class BulkUpdateRequest(BaseModel):
    ...
    updates: List[BulkUpdateItem] = Field(
        ..., description="A list of update operations to perform"
    )
...
class CutPayAgentConfigCreate(BaseModel):
    ...
    payment_mode_detail: Optional[str] = Field(
        None, description="Additional payment details"
    )
...
class CutPayAgentConfigUpdate(BaseModel):
    ...
    payment_mode: Optional[str] = Field(
        None, description="Payment mode (NEFT, Cash, Cheque)"
    )
    payment_mode_detail: Optional[str] = Field(
        None, description="Additional payment details"
    )
    po_paid_to_agent: Optional[float] = Field(
        None, ge=0, description="PO amount paid to agent"
    )
