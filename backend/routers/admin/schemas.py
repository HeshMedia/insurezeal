...
class ChildIdAssignment(BaseModel):
    ...
    child_id: str = Field(
        ..., min_length=1, max_length=50, description="Unique child ID to assign"
    )
    ...
    password: Optional[str] = Field(
        None, description="Password for child ID assignment"
    )
...
class ChildIdStatusUpdate(BaseModel):
    ...
    admin_notes: str = Field(
        ...
        min_length=1,
        max_length=1000,
        description="Required reason for status change",
    )
