"""
Child ID Management Helper Functions for Insurezeal Backend API.

This module provides comprehensive helper functions for managing child ID
requests, broker relationships, and insurer associations. It handles the
complex workflow of child ID creation, validation, approval processes,
and integration with broker and insurance company systems.

Key Features:
- Child ID request creation and validation
...
- Broker capacity and eligibility validation
- Insurance company association and verification
- Request approval and rejection processes
- Data integrity and relationship validation
- Audit trails for all child ID operations

Database Operations:
- Child ID request CRUD operations
- Broker and insurer data management
...
from models import ChildIdRequest, Broker, Insurer
...
class ChildHelpers:
    """
    Comprehensive helper class for child ID management operations.

    This class provides methods for creating, managing, and processing child ID
    requests, including validation, approval workflows, and integration with
    broker and insurer systems. It ensures data integrity and business rule
    compliance throughout the child ID lifecycle.
    ...
    Key Responsibilities:
        ...
        - Integration with approval workflows
        - Error handling and logging for operations
    """
    ...
    async def create_child_id_request(
        ...
        try:
            ...
            child_request = ChildIdRequest(user_id=uuid.UUID(user_id), **request_data)
    ...
    async def get_user_child_requests(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Invalid UUID format for user_id: '{user_id}' - Length: {len(user_id)} - Error: {str(uuid_error)}"
                )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            if user_id:
                try:
                    ...
                    logger.error(
                        f"Invalid UUID format for user_id: '{user_id}' - Error: {str(uuid_error)}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail=f"Invalid user ID format. Expected valid UUID, got: '{user_id}'",
