"""
Role-Based Access Control (RBAC) Dependencies for Insurezeal Backend API.

This module implements a comprehensive role-based access control system
for the FastAPI insurance platform. It provides fine-grained permission
management across different user roles and resources, ensuring secure
access to platform functionality based on user privileges.

Key Features:
- Hierarchical role-based permission system
- Resource-specific access control (read, write, update, delete, manage)
- FastAPI dependency integration for route protection
- Dynamic permission validation based on user context
- Comprehensive role definitions for insurance platform

Role Hierarchy:
1. superadmin: Full platform access including user management
2. admin: Broker/organization management and agent oversight
3. agent: Policy management and client interaction
4. user: Basic policy viewing and document access

Resource Categories:
- superadmin: Platform administration and global settings
- admin: Broker management, agent oversight, reporting
- users: User profile and document management
- child: Child ID requests and management
- policies: Policy creation, editing, and management
- mis: Management information system reports

Security Model:
- Dependency functions run after authentication
- Permission checking based on user role and requested resource
- Granular action-level permissions (read/write/update/delete/manage)
- Context-aware access control with request validation
...
- User role extraction from authenticated sessions
- Resource mapping to permission requirements
"""
